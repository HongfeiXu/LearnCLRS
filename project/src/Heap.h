...
*  @date       8.16 2017
*  @remark     Implementation MaxHeap and MinHeap
...
public:
	...
	MaxHeap(KeyType A_[], int heap_size_)
	{
	...
	MaxHeap(KeyType A_[], DataType Data_[], int heap_size_)
	{
		heap_size = heap_size_;
		for (int i = 1; i <= heap_size; i++)
		{
	...
	**/
	...
	void HeapIncreaseKey(int i, KeyType key)
	{
		if (key < A[i])
		{
...
public:
	...
	MinHeap(KeyType A_[], int heap_size_)
	{
		heap_size = heap_size_;
		for (int i = 1; i <= heap_size; i++)
			A[i] = A_[i];
	...
	MinHeap(KeyType A_[], DataType Data_[], int heap_size_)
	{
		heap_size = heap_size_;
		for (int i = 1; i <= heap_size; i++)
		{
	...
	void HeapInsert(KeyType key, DataType data_)
	{
		heap_size++;
		A[heap_size] = 1000000000;        // 增加一个关键字为 1000000000 的叶节点来扩展最小堆
		Data[heap_size] = data_;
		HeapDecreaseKey(heap_size, key);  // 调用 HeapIncreaseKey 为新节点设置关键字，同时保存最大堆的性质
	}
};
...
void HeapTest()
{
	...
	cout << "After HeapDecreaseKey():" << endl;
	for (int i = 1; i <= min_heap.heap_size; i++)
	{
		cout << min_heap.A[i] << " ";
	}
